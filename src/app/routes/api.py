...
from app.bootstrap.container import get_context_container, Mode
...
def create_api_router(prefix: str, tags: list[str]) -> APIRouter:
    ...
    async def get_ship_status(id: int):
        async with get_context_container(Mode.API) as container:
            ship = await container.ship_service.find(id)
            if ship is None:
    ...
    async def command(payload: GameCommandRequest):
        async with get_context_container() as container:
            ship = await container.ship_service.find(payload.entity_id)
            if not ship:
                raise ShipNotFound(payload.entity_id)
